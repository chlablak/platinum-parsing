...
\begin{document}
	...
	\label{sec:creation-d-un-parser-pour-la-grammaire}
		...
		\begin{figure}[H]
			\centering
			\begin{tabular}{ll}
				\texttt{PP} & Import global des fonctionnalités de PP\\
				\texttt{PP.Grammar} & Définition des fonctions communes aux différentes grammaires (classe de type)\\
				\texttt{PP.Grammars.Ebnf} & Définition de la grammaire EBNF (AST, \textit{parsers}, instances)\\
			\end{tabular}
		...
		\todo{expliquer \texttt{PP.Grammar.InputGrammar}}
		...
		\subsection{Test avec des grammaires}
			...
			\begin{figure}[H]
				\centering
				\begin{tabular}{ll}
					\texttt{Spec} & Regroupement des différents tests\\
					\texttt{PPTest.Grammars.Ebnf} & Tests pour la grammaire EBNF\\
				\end{tabular}
				\caption{Premiers modules de test}
			\end{figure}  
			
			\todo{expliquer \texttt{Hspec}, utilité de \texttt{stringify}}
		...
		\subsection{Intégration au CLI}
			...
			\begin{figure}[H]
				\centering
				\begin{tabular}{ll}
					\texttt{Main} & Traitement des arguments et délégation à la bonne commande\\
					\texttt{Args} & Structure Haskell représentant les arguments possibles\\
					\texttt{Cmd.Ebnf} & Implémentation des fonctionnalités de la commande \texttt{pp ebnf}\\
				\end{tabular}
			...
			Ce qui correspond au \textit{parsing} du fichier \texttt{doc/grammars/ebnf.ebnf} en AST, puis on \texttt{stringify} l'AST et on l'affiche.
	...
	\section{Génération de la table LALR}		
		...
		\begin{enumerate}
			...
			\subitem soit une fonction \mintinline{haskell}{toRules :: Ebnf.Syntax -> [Rule]} 
			\subitem avec \mintinline{haskell}{type Rule = (String, [Part])} 
			\subitem et \mintinline{haskell}{data Part = Term Char | NonTerm String | Empty}
			
			\item augmenter la grammaire (\mintinline{haskell}{[Rule]}) pour avoir la grammaire augmentée
			\subitem soit une fonction \mintinline{haskell}{augmente :: [Rule] -> Either String [Rule]}
			\subitem (on utilise le type \mintinline{haskell}{Either} car la fonction peut échouer)
			
			\item générer les collections d'items LR(1) canonique pour la grammaire
			\subitem soit une fonction \mintinline{haskell}{generateSet :: [Rule] -> Either String Lr1Set}
			\subitem avec \mintinline{haskell}{type Lr1Set = Array Integer (Set Lr1Item)}
			\subitem et \mintinline{haskell}{type Lr1Item = (Rule, Integer, Part)}
			\subitem et qui aura besoin des fonctions suivantes:
			\subitem \mintinline{haskell}{closing :: (Set Lr1Item) -> (Set Lr1Item)}
			\subitem \mintinline{haskell}{goto :: (Set Lr1Item) -> Part -> (Set Lr1Item)}
			\subitem \mintinline{haskell}{first :: Part -> [Part]}
			
			\item fusion de la collection LR(1) pour avoir une collection LALR(1) de plus petite taille
			\subitem soit une fonction \mintinline{haskell}{fusion :: Lr1Set -> Either String LalrSet}
			\subitem avec \mintinline{haskell}{type LalrSet = Lr1Set}
			
			\item génération de la table LALR proprement dite
			\subitem soit une fonction \mintinline{haskell}{generateTable :: LalrSet -> Either String LalrTable}
			\subitem avec \mintinline{haskell}{type LalrTable = Array (Integer, Part) Action}
			\subitem et \mintinline{haskell}{data Action = Shift ... | Reduce ... | Goto ... | Error ... | Accept}\\
		\end{enumerate}
		
		Si on prend en compte les dépendances entre les fonctions, nous obtenons l'ordre dans lequel nous devons implémenter chacune: \texttt{toRules}, \texttt{augmente}, \texttt{first}, \texttt{closing}, \texttt{goto}, \texttt{generateSet}, \texttt{fusion} et pour finir \texttt{generateTable}.\\
		
		Voyons donc en premier la fonction \texttt{toRules}, qui transforme l'AST de EBNF en AST simplifié, tout en gardant le sens original des règles EBNF \cite{ebnftobnf}. Par exemple les règles \mintinline{ebnf}{a = [b]; b = "c" | "d";} seront transformées en \mintinline{ebnf}{a = b; a = ; b = 'c'; b = 'd';}. Pour cela nous définissons le type \texttt{Rule} dans \texttt{PP.Rule}, et nous ajoutons une fonction \mintinline{haskell}{rules :: ast -> [Rule]} à la classe de type \texttt{PP.Grammar.InputGrammar}. De ce fait nous devons implémenter cette fonction pour l'AST de l'EBNF, cela se fait dans \texttt{PP.Grammars.Ebnf}. Les tests sont ajoutés au module \texttt{PPTest.Grammars.Ebnf} naturellement.\\
