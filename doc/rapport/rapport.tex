...
\begin{document}
	...
	\section{Génération de la table LALR}		
		...
		À présent que nous avons l'AST d'une grammaire, nous pouvons nous attaquer à la génération de la table LALR correspondante. Ce processus est assez complexe, nous allons donc d'abord séparer les différentes fonctions utiles et structures de données utilisées (informatif):
		\begin{enumerate}
			\item transformation des règles EBNF en règles canoniques
			\subitem soit une fonction \hs{rules :: Ebnf.Syntax -> [Rule]} 
			\subitem avec \hs{type Rule = (String, [Part])} 
			\subitem et \hs{data Part = Term Char | NonTerm String | Empty}
			...
			\subitem soit une fonction \hs{generateSet :: [Rule] -> Either String Lr1Set}
			\subitem avec \hs{type Lr1Set = Array Integer (Set Lr1Item)}
			\subitem et \hs{type Lr1Item = (Rule, Integer, Part)}
			\subitem et qui aura besoin des fonctions suivantes:
			\subitem \hs{closing :: (Set Lr1Item) -> (Set Lr1Item)}
			\subitem \hs{goto :: (Set Lr1Item) -> Part -> (Set Lr1Item)}
			\subitem \hs{first :: Part -> [Part]}
			
			\item fusion de la collection LR(1) pour avoir une collection LALR(1) de plus petite taille
			\subitem soit une fonction \hs{fusion :: Lr1Set -> Either String LalrSet}
			\subitem avec \hs{type LalrSet = Lr1Set}
			
			\item génération de la table LALR proprement dite
			\subitem soit une fonction \hs{generateTable :: LalrSet -> Either String LalrTable}
			\subitem avec \hs{type LalrTable = Array (Integer, Part) Action}
			\subitem et \hs{data Action = Shift ... | Reduce ... | Goto ... | Error ... | Accept}\\
		\end{enumerate}
		
		Si on prend en compte les dépendances entre les fonctions, nous obtenons l'ordre dans lequel nous devons implémenter chacune: \hs{rules}, \hs{extend}, \hs{first}, \hs{closing}, \hs{goto}, \hs{generateSet}, \hs{fusion} et pour finir \hs{generateTable}.\\
		...
		Passons à la fonction \hs{extend :: [Rule] -> Either String [Rule]}. Celle-ci doit augmenter la grammaire, c'est-à-dire pour une grammaire \mintinline{ebnf}{a = b; b = 'c';} on aura: \mintinline{ebnf}{__start = a; a = b; b = 'c';}. Il est possible que la grammaire ne contienne aucune ou plusieurs règles de départ, cela doit être détecté évidemment. L'implémentation se trouve dans \hs{PP.Rule} et les tests dans \hs{PPTest.Rule}.\\
		
		\todo{parler de RuleSet\\}
		...
		\paragraph{}
		À présent nous pouvons attaquer un nouveau module, \hs{PP.Builders.Lr1}, avec la fonction \hs{closing} et les types \hs{Lr1Item} et \hs{Lr1Set}. \todo{}\\	
