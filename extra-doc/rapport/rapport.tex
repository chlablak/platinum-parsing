...
\begin{document}
	...
	\section*{Table des abréviations}
		...
		\end{itemize}
		\todo{vérifier le rapport pour d'autres abréviations}
	\newpage
	...
	\section{Génération de la table LALR}
		\todo{plus de détails sur les sous-tâches}		
	...
	\section{Test d'une grammaire à partir d'un texte en entrée}
		...
		\label{subsec:test-grammaire-integration-au-cli}
			...
			Comme exemples:
			\begin{minted}{console}
				prompt$ pp ebnf -f doc/grammars/wrong/03-direct-left-recursion.ebnf --check
				[PP][INFO] starting...
				...
				prompt$ pp lalr -f doc/grammars/test.ebnf --test-with ignored/test.txt 
				[PP][INFO] starting...
				[PP][INFO] verbosity: 30
	...
	\section{Génération des tables dans un langage cible}
		...
		\begin{figure}[H]
			...
			\begin{tabular}{llll}
				...
				\hline
				LrContext & states & [LrContextState] & States informations\\
				...
				LrContextState & id & Int & State ID\\
				 & alt & LrContextNonTerm & Associated non-terminal (not impl. yet)\\
				LrContextTerm & symbol & Char & Terminal symbol\\
				 & isEmpty & Bool & It's the EMPTY symbol?\\
				LrContextNonTerm & name & String & Non-terminal name\\
				 & length & Int & Rule length (right side)\\
				LrContextTable & rows & [LrContextTableRow] & Table flatten in rows only\\
				 & total & Int & Total rows (with errors)\\
				LrContextTableRow & state & LrContextState & Row state\\
				...
				LrContextAction & isReduce & Bool & Is action reduce?\\
		...
		\subsection{Intégration au CLI}
			...
			\begin{minted}{console}
				prompt$ pp -s lalr -f grammar.ebnf --template template.foo > output.foo
			\end{minted}
	...
	\section{Démonstration en C\#}
		...
		\subsection{Validation du \textit{pipeline}}
			...
			\begin{minted}{text}
				prompt$ pp -s lalr -f simple.ebnf --template csharp.cs > LrTableImpl.cs
				prompt$
			\end{minted}
		...
		\subsection{Démonstration finale}
			\todo{pipeline, Program.cs}
			
	\section{TODO}
		\todo{organisation du dossier du projet}
		\todo{modules finaux de PP}
		\todo{état actuel de PP, futur...}
		\todo{introduction / remerciements}
		\todo{conclusion}
	
	\section{Annexes}
			
		\subsection{Table des figures et tableaux}
			\begingroup
