...
\begin{document}
	...
	\label{sec:architecture-du-projet}
		...
		\subsection{GOLD Parser System \cite{goldparser}}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.8\textwidth]{../../res/diagrams/pipeline/pipeline-gold-parser}
		...
		\subsection{Organisation de Platinum Parsing}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.8\textwidth]{../../res/diagrams/pipeline/pipeline-output} 
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.8\textwidth]{../../res/diagrams/pipeline/pipeline} 
		...
		\subsection{Séparation des fonctionnalités framework/CLI}
			...
			\subsubsection{Cas 1: interprétation du langage Lua}
				...
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.8\textwidth]{../../res/diagrams/pipeline/pipeline-framework} 
	...
	\label{sec:creation-d-un-parser-pour-la-grammaire}
		...
		\subsection{Intégration au CLI}
			...
			\paragraph{}
			Ce qui correspond au \textit{parsing} du fichier \texttt{res/grammars/ebnf.ebnf} en AST, puis on \hs{stringify} l'AST et on l'affiche.
	...
	\label{sec:generation-de-la-table-lalr}	
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.6\textwidth]{../../res/diagrams/misc/generation-process}
	...
	\label{sec:test-d-une-grammaire-a-partir-d-un-texte-en-entree}
		...
		\label{subsec:test-grammaire-integration-au-cli}
			...
			\begin{minted}{console}
				...
				$ pp lalr -f res/grammars/test.ebnf --test-with ignored/test.txt 
				[PP][INFO] starting...
				[PP][INFO] verbosity: 30
	...
	\section{Intégration de l'analyse lexicale}
		...
		\subsection{Séparation des parties \textit{lexer} et des parties \textit{parser}}
			Cette partie a pu être faîte dans la partie précédente.
		...
		\subsection{Implémentation du \textit{lexer} complet}
			L'architecture d'un \textit{lexer} est semblable à celle d'un \textit{parser} LR:
			\begin{figure}[H]
				\centering
		...
		\subsection{Intégration à PP et tests}
			...
			Les responsabilités étant bien découplés, le support des \hs{OToken} n'ont pas demandé beaucoup de changements.\\
			...
			En plus des bugs découverts et corrigés, nous avons constater que la génération de la table LALR et de l'AFD prenait environ 45 minutes. Ce temps est conséquent, surtout si nous devons relancer une commande plusieurs fois et donc régénérer le tout à chaque fois. Cette problématique est le sujet de la prochaine section.
	...
	\section{Réutilisation des résultats précédents}
		...
		\begin{table}[H]
			...
			\begin{tabular}{ll}
				...
				\hs{Data.Yaml} & Manipulation de fichier \texttt{.yaml}\\
				\hs{System.Directory} & Manipulation du système de fichiers\\
			\end{tabular}
			\caption{Bibliothèques utiles pour le module \hs{Work} du CLI}
		\end{table}
			
		Le module stocke ces informations dans le dossier \texttt{.pp-work/}, relativement au dossier où est lancé la commande. De plus le fichier \texttt{.pp-work/work.yaml} contient toutes les informations dont le module a besoin:
		\begin{minted}{yaml}
		...
		Tout cela permet de gagner un temps énorme lorsque la grammaire n'est plus amenée à changer. Notons que l'utilisateur a le choix de l'utiliser ou non à l'aide de l'option \texttt{-w} du CLI.
		...
		\subsection{Analyse des performances de la génération}
			...
			\begin{enumerate}
				\item La collection est intrinsèquement de grande taille
				\item L'algorithme utilisé est connu pour être gourmand
	...
	\section{Intégration à un éditeur de texte}
		...
		\begin{figure}[H]
			...
			\caption{Création d'un nouveau projet PP}
		\end{figure}
		...
		On voit que les informations du fichier de configuration ont bien été prises en compte. Notons également le dossier \texttt{.pp-work/} qui a été utilisé.\\
		...
		\begin{figure}[H]
			...
			\caption{Test de la grammaire sur un fichier source avec Atom}
		\end{figure}
	...
	\section{Mise en ligne du projet}
		...
		\subsection{GitHub}
			Pour finir le \textit{repo} du projet a été mis en public: \url{https://github.com/chlablak/platinum-parsing}. Dans le but que PP soit un projet sérieux, un \textit{README} avec procédure d'installation et liens utiles a été fait.\\
			
			De plus, nous avons ajouté un dossier \texttt{doc/} contenant des exemples d'utilisation, des références ainsi que des procédures utiles pour une personne souhaitant travailler sur Platinum Parsing.
		
	\section{Démonstration de PP avec une grammaire conséquente}
		\todo{}	
