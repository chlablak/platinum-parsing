...
namespace csharp_lexer_analysis.engine_with
{
    ...
    class LrParser
    {
         ...
         */
        public LrParser(LrTable table, List<Token> input)
        {
            this.table = table;
            LrAction action = table.Action(0, input.Count > 0 ? input[0] : table.Empty());
            ...
            switch (config.action.type)
            {
                case LrAction.Type.Shift:
                    ...
                    config.action = table.Action(config.action.value,
                        config.input.Count > 1 ? config.input[1] : table.Empty());
                    if (config.input.Count > 0)
                    {
                        ast.Shift(config.input[0].value);
                        config.input.RemoveAt(0);
                    }
                    break;
                ...
                case LrAction.Type.Goto:
                    ...
                    config.stack.Push(config.action.value);
                    config.action = table.Action(config.action.value,
                        config.input.Count > 0 ? config.input[0] : table.Empty());
                    break;
